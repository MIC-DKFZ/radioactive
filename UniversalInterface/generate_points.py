...
for gt_mask_path in tqdm(test_label_paths):
    ...
    for label, organ in tqdm(fg_labels_dict.items()):
        ...
        volume_fg = tuple(arr.astype(int) for arr in volume_fg) # recast to permit JSON serialisation
        ...
        if n_fg_voxels == 0:
            ...
            prompt_dict[label]['3D'] = []
            prompt_dict[label]['2D'] = []
            continue
        ...
        points3D = [((int(volume_fg[0][idx])), int(volume_fg[1][idx]), int(volume_fg[2][idx])) for idx in point_indices] # change from triple of arrays format to list of triples format
        prompt_dict[label]['3D'] = points3D
        ...
        for slice_index in fg_slices:
            ...
            slice_fg = tuple(arr.astype(int) for arr in slice_fg) # recast to permit JSON serialisation
            ...
            if n_fg_pixels >= n_clicks:
                ...
                warning_zs[f'x = {slice_index}'] = n_fg_pixels
                point_indices = np.concatenate([np.arange(n_fg_pixels),
                                            np.random.choice(n_fg_pixels, size = n_clicks-n_fg_pixels, replace = True)])
            
            points2D.extend([(int(slice_index), int(slice_fg[0][idx]), int(slice_fg[1][idx])) for idx in point_indices]) #Recast as native integers to permit json serialisability
        ...
        prompt_dict[label]['2D'] = points2D

    full_prompt_dict[os.path.basename(gt_mask_path)] = dict(prompt_dict)

os.makedirs(res_dir, exist_ok = True)
with open(os.path.join(res_dir, 'prompts.json'), 'w') as f:
    json.dump(full_prompt_dict, f, indent = 4)
