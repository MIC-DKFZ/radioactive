...
def run_experiments_2d(inferer, imgs_gts, results_dir, save_segs = False):
    ...
    if save_segs:
        dir_list = [os.path.join(results_dir, os.path.basename(gt).removesuffix('.nii.gz')) for (img, gt) in imgs_gts]
        for dir in dir_list:
            os.makedirs(dir, exist_ok = True)


    # Initialize results dictionary
    results = {os.path.basename(gt): {} for (img, gt) in imgs_gts}
    ...
    for img_path, gt_path in tqdm(imgs_gts, desc = 'looping through files\n', leave = False):
        ...
        for instance in tqdm(instances_present, leave = False):
            ...
            results[base_name][str(instance)] = dice_score
            
            if save_segs:
                seg_orig_ori = inv_transform(segmentation) # Reorient segmentation
                save_path = os.path.join(results_dir, base_name.removesuffix('.nii.gz'), 'instance_' + str(instance) + '_seg.nii.gz')
