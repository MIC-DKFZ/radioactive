...
import utils.prompt_3d as prompt_3d
...
def get_experiments(inferer, seed):
    ...
    if 'point' in inferer.supported_prompts:
        ...
        if inferer.dim == 3:
            experiments.update({
                'random_points_3d': lambda organ_mask: prompt_3d.get_pos_clicks3D(organ_mask, 1, seed)
            })
    ...
    if 'box' in inferer.supported_prompts:
        ...
        if inferer.dim == 3:
            experiments.update({
                'bbox3d': lambda organ_mask: prompt_3d.get_bbox3d(organ_mask)
            })
    return experiments

def run_experiments(inferer, imgs_gts, dataset_dir, results_dir, target_is_lesion, save_segs = False, seed = 11121, labels_dict_override = None):
    ...
    # Find fg labels for organs/number of lesions:
    if target_is_lesion:
        instances_dict_path = os.path.join(dataset_dir, 'instances.json')
        if not os.path.exists(instances_dict_path):
            raise RuntimeError('Must run preprocessing first to find which lesions are in which ground truths')
        with open(instances_dict_path, 'r') as f:
            instance_dictionary = json.load(f)
    else:
        dataset_json_path = os.path.join(dataset_dir, 'dataset.json')
        if not os.path.exists(instances_dict_path):
            raise RuntimeError('Dataset directory must include a dataset json containing a labels dictionary detailing the organs')
        with open(dataset_json_path, 'r') as f:
            labels_dict = json.load(f)["labels"]
        if labels_dict_override:
            labels_dict = labels_dict_override


    # Generate segmentations
    overall_status = 'All segmentations successfully generated'
    status_dict = {exp_name: overall_status for exp_name in exp_names}
    ...
    for split in ['Tr', 'Ts']:
        for img_path, gt_path in tqdm(imgs_gts[split], desc = 'looping through files\n', leave = False):
            ...
            if target_is_lesion:
                targets = instance_dictionary[split][os.path.basename(gt_path)]
            
            # Loop through each instance of lesion present
            for target in tqdm(targets, leave = False, desc = 'looping through instances'):
                organ_mask = np.where(gt == target, 1, 0)
                ...
                for exp_name, prompting_func in experiments.items():
                    ...
                    try:
                        ...
                        if save_segs:
                            seg_dir = os.path.join(results_dir + '_' + exp_name, 'segmentations' + split, base_name.removesuffix('.nii.gz'))
                            os.makedirs(seg_dir, exist_ok = True)
                            segmentation.to_filename(os.path.join(seg_dir, 'instance_' + str(target) + '_seg.nii.gz'))

                    except Exception as e: 
                        dice_score = None
                        status_dict[exp_name] = overall_status ='Some segmentations failed'
                        loggers[exp_name].error(f'Segmenting instance {target} in image {base_name} failed: {str(e)}')
                    results[exp_name][split][base_name][str(target)] = dice_score
    ...
    # Log overall status per experiment
    for exp_name in exp_names:
        loggers[exp_name].info(status_dict[exp_name])
        
    print(f"Segmentations generated and saved to {[results_dir + '_' + exp_name for exp_name in exp_names]}; {overall_status}")
...
def run_postprocess(results_dir, exp_names, dataset_dir):
    ...
    for exp_name in exp_names:
        ...
        for split in ['Tr', 'Ts']:
            ...
            seg_dir_parent = os.path.join(exp_results_dir, 'segmentations' + split)
            if not os.path.exists(seg_dir_parent):
                continue
            
            seg_dirs = [os.path.join(seg_dir_parent, f) for f in os.listdir(seg_dir_parent)] # Obtain all segmentation folders (ie each one containing all the instance segmentations for a given image
            seg_dirs = [d for d in seg_dirs if not d.endswith('merged')] # Ignore merged folders from possible previous attempts at postprocessing
            ...
            for seg_dir in seg_dirs:
                ...
                if num_instances_segmented == 0:
                    continue # Skip if there are no instances segmented
