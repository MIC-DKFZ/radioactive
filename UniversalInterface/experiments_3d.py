...
def run_experiments_3d(inferer, imgs_gts, results_dir, label_dict,
                    exp_params, prompt_types,
                    seed, experiment_overwrite = None, save_segs = False):
    ...
    if save_segs:
        dir_list = [os.path.join(results_dir, exp_name, target) for exp_name in experiment_names for target in targets]
        for dir in dir_list:
        ...
        for target, target_label in tqdm(label_dict.items(), desc = 'looping through organs\n', leave = False):
            ...
            for exp_name, prompting_func in tqdm(experiments.items(), desc = 'looping through non_interactive experiments', leave = False):
                ...
                if save_segs:
                    ...
                    save_path = os.path.join(results_dir, exp_name, target, os.path.basename(gt_path).replace('.nii.gz','_seg.nii.gz'))
            ...
            for exp_name, prompting_func in tqdm(interactive_experiments.items(), desc = 'looping through interactive experiments', leave = False):
                ...
                if save_segs:
                    dice_scores, dofs = iterate_3d(inferer, img, gt, inferer.pass_prev_prompts, 
                                                exp_params.perf_bound, exp_params.dof_bound, seed)
                else:
                    dice_scores, dofs, segmentations, prompts = iterate_3d(inferer, img, gt, inferer.pass_prev_prompts, 
                                                exp_params.perf_bound, exp_params.dof_bound, seed, detailed = True)
                results[exp_name][target_label][base_name] = {'dof': dofs, 'dice_scores': dice_scores}      

                if save_segs:
                    for i, segmentation in enumerate(segmentations):
                        seg_orig_ori = inv_transform(segmentation) # Reorient segmentation
                        save_path = os.path.join(results_dir, exp_name, target, base_name).replace('.nii.gz', f'_seg_{i}.nii.gz')
                        seg_orig_ori.to_filename(save_path)         
