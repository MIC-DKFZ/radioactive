...
import torch
from .base_classes import Points
...
def iterate_2d(inferer, img, gt, segmentation, initial_prompt, 
               condition, dof_bound = float('inf'), perf_bound = float('inf'), init_dof = 0, detailed = False, seed = None):
    if seed:
        np.random.seed(seed)
    ...
    while True:
        ...
        new_seg = inferer.predict(img, new_prompt)
        prompts.append(new_prompt)
        segmentation[improve_slices] = new_seg[improve_slices]
        segmentations.append(segmentation.copy())

        # Update the trackers
        low_res_masks.update({fix_slice_idx: torch.sigmoid(inferer.slice_lowres_dict[fix_slice_idx]).squeeze().cpu().numpy() for fix_slice_idx in improve_slices})
        dice_scores = [prUt.compute_dice(segmentation, gt)]
        print(dice_scores[-1])
    ...
    if detailed == False:
        return segmentation, dof
    else:
        return segmentation, dof, segmentations, prompts, max_fp_idxs