            ...
            for instance in tqdm(instances_present, leave = False, desc = 'looping through instances'):
                ...
                for exp_name, prompting_func in experiments.items():
                    ...
                    try:
                        ...
                        if save_segs:
                            seg_dir = os.path.join(results_dir + '_' + exp_name, exp_name, 'segmentations' + split, base_name.removesuffix('.nii.gz'))
                            os.makedirs(seg_dir, exist_ok = True)
                            segmentation.to_filename(os.path.join(seg_dir, 'instance_' + str(instance) + '_seg.nii.gz'))

                    except Exception as e: 
                        dice_score = None
                        overall_status = 'Some segmentations failed'
                        loggers[exp_name].error(f'Segmenting instance {instance} in image {base_name} failed: {str(e)}')
