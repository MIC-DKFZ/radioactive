...
def run_inference_for_inferer(inferer, image_paths, labels_paths, organ_labels_dict, n_points, seed):
    ...
    for img_path, gt_path in tqdm(zip(image_paths, labels_paths), desc = 'Inferring'):
        for organ, organ_label in organ_labels_dict.items():
            ...
            pts_prompt = prUt.get_pos_clicks2D_row_major(gt, n_points, seed = seed)
            
            # Segment and store results
            try:
                segmentation = inferer.predict(img, pts_prompt)
            except:
                print(f'Segmentation failed for {img_path} {organ}. Giving dice_score NaN')
                res_dict[organ][os.path.basename(img_path)] = np.nan
                continue
...
def run_expnpointsperslice(dataset_path, n_points, save_dir, organ_labels_dict = None, seed = 11121, device = 'cuda'):
    ...
    save_path = os.path.join(save_dir, f'exp{n_points}pointsperslice_sam.json')
    with open(save_path, 'w') as f:
        json.dump(res_dict_sam, f, indent=4)
...
if __name__ == '__main__':
    ...
    save_dir = '/home/t722s/Desktop/inferenceResults'
    organ_labels_dict = {
                        "aorta": 1,
                        "gall_bladder": 2,
                        "kidney_left": 3,
    ...
    run_expnpointsperslice(dataset_path, 1, '/home/t722s/Desktop/inferenceResults/exp1pointperslice.json', organ_labels_dict, 11121)
    run_expnpointsperslice(dataset_path, 5, '/home/t722s/Desktop/inferenceResults/exp5pointperslice.json', organ_labels_dict, 11121)