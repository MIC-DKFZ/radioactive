...
class PromptStep:
    ...
    def set_points(self, point_prompts: Points | tuple[np.ndarray, np.ndarray] | None):
        assert isinstance(
            point_prompts, (Points, tuple, type(None))
        ), f"must be supplied as a pair (coords, labels), with coords nx3 containing the coordinates (xyz) of the prompts, or as a Points object. Got {type(point_prompts)}"

        if isinstance(point_prompts, Points):
            assert len(point_prompts.coords) == len(
                point_prompts.labels
            ), "Labels and coordinates do not match up - Not the same number of each."
            self.coords, self.labels = point_prompts.coords, point_prompts.labels
            self.has_points = True
        elif isinstance(point_prompts, tuple):
            assert len(point_prompts[0]) == len(
                point_prompts[1]
            ), "Labels and coordinates do not match up - Not the same number of each."
    ...
    def set_boxes(self, box_prompts: Boxes | dict[int, np.ndarray] | None):
        assert isinstance(
            box_prompts, (Boxes, dict, type(None))
        ), f"Boxes must be supplied as a dictionary of z_slice: (x_min, y_min, x_max, y_max) or as a Boxes object. Got {type(box_prompts)}"
    ...
    def set_masks(self, mask_prompts: dict[int, np.ndarray] | np.ndarray | None):
        assert isinstance(
            mask_prompts, (dict, np.ndarray, type(None))
        ), "Mask prompts must be given either as a numpy array for 3d images, or as a dict of z_slice: slice_mask for 2d images"
...
def _merge_dict_prompts(
    prompt_dict_1: dict[int, np.ndarray] | None, prompt_dict_2: dict[int, np.ndarray] | None
    ...
    for slice_idx, prompt in prompt_dict_1.items():
        if slice_idx in prompt_dict_2.keys() and not np.array_equal(
            prompt_dict_1[slice_idx], prompt_dict_2[slice_idx]
        ):
            raise ValueError(
                "Merging would cause having two distinct box/mask prompts on one slice, which is not permitted"
            )
...
def _merge_point_prompts(
    coords1: np.ndarray | None, labels1: np.ndarray | None, coords2: np.ndarray | None, labels2: np.ndarray | None
...
def merge_sparse_prompt_steps(prompt_steps: list[PromptStep]) -> PromptStep:
    ...
    assert isinstance(
        prompt_steps[0], PromptStep
    ), f"Expected elements of prompt_steps to be of type PromptStep, got {type(prompt_steps[0])}"
