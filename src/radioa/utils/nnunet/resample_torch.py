...
from radioa.utils.nnunet.configuration import ANISO_THRESHOLD
from radioa.utils.nnunet.simpleitk_reader_writer import SimpleITKIO
from radioa.utils.nnunet.default_resampling import determine_do_sep_z_and_axis
    ...
    if all([i == j for i, j in zip(new_shape, data.shape[1:])]):
        ...
        with torch.no_grad():
            ...
            if is_seg:
                ...
                unique_values = unique_values.to(result_dtype)
                ...
                if not memefficient_seg_resampling:
                    ...
                    for i, u in enumerate(unique_values):
                        if u == 0:
                            pass
                        result[
                            F.interpolate((data[None] == u).float(), new_shape, mode=torch_mode, antialias=False)[0]
                            > 0.5
                        ] = u
            else:
