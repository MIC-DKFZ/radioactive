...
from radioa.utils.resample import get_current_spacing_from_affine
...
class SAM2Transforms(nn.Module):
    ...
    def transform_coords(self, coords: torch.Tensor, normalize=False, orig_hw=None) -> torch.Tensor:
        ...
        )  # unnormalize coords # Tim: I don't thinkk this will return coordinates in the range [0,1].....
        return coords
...
def resample_to_shape_sparse(
    ...
    """
    coords = (
        coords * (np.array(target_shape) - 1) / (np.array(current_shape) - 1)
    )  # -1 since coords can range in value from 0 to shape-1
    if round:
