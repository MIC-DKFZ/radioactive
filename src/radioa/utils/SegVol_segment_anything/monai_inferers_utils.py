...
def sliding_window_inference(
    ...
    **kwargs: Any,
    ...
    if use_point or use_box:
        ...
        global_preds = F.pad(
            global_preds, pad=pad_size, mode=look_up_option(padding_mode, PytorchPadMode).value, value=cval
        )
    #############
    ...
    for slice_g in tqdm(range(0, total_slices, sw_batch_size)) if progress else range(0, total_slices, sw_batch_size):
        ...
        points = None
        if use_point:
            window_binary_prompt_map = torch.cat([binary_prompt_map[win_slice] for win_slice in unravel_slice]).to(
                sw_device
            )
