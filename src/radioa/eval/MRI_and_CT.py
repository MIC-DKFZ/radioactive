...
def generate_latex(df: pd.DataFrame, grey_shades: Dict[str, str]) -> str:
    ...
    df['Average MRI'] = df[['D1', 'D2', 'D3']].mean(axis=1).round(2)
    df['Average CT'] = df[['D4', 'D5', 'D6', 'D7', 'D8', 'D9', 'D10']].mean(axis=1).round(2)


    # Generate LaTeX table
    latex_rows = ['\\begin{tabular}{llllrrr}', '\\toprule']
    latex_rows.append('Prompter & Model & Interactions & ' + ' & '.join(df.columns[13:]) + ' \\\\')
    ...
    for _, row in df.iterrows():
        ...
        # Format row values
        row_values = ' & '.join([str(row[col]) if pd.notna(row[col]) else 'NaN' for col in df.columns[:3]])
        row_values += '&'
        row_values += ' & '.join([str(row[col]) if pd.notna(row[col]) else 'NaN' for col in df.columns[13:]])
