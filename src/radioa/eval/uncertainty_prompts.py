...
def generate_latex(df: pd.DataFrame, grey_shades: Dict[str, str]) -> str:
    ...
    desired_prompters = ['3B Inter',  '3B Inter $\pm$3',  '3B Inter $\pm$5']
    ...
    latex_rows = ['\\begin{tabular}{lllrrrrrrrrrrr}', '\\toprule']
    latex_rows.append('Model & Promter & Interactions &' + ' & '.join(df.columns[3:]) + ' \\\\')
...
if __name__ == '__main__':
    ...
    realistic_prompters: List[str] = [
        prompter_names[p][0] for p in ['ThreeBoxInterpolationPrompter','ThreeBoxInterpolationPrompter_nonperfect3', 'ThreeBoxInterpolationPrompter_nonperfect5',
                                       ]
    ]
    ...
    latex_table: str = generate_latex(sorted_df[sorted_df['Model'] =='SAM2'], GREY_SHADES)
    print(latex_table)
