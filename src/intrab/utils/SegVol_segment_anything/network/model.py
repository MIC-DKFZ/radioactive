...
from intrab.utils.SegVol_segment_anything.loss import BCELoss, BinaryDiceLoss
...
class SegVol(nn.Module):
    ...
    def forward(self, image, text=None, boxes=None, points=None, **kwargs):
        ...
        image_embedding = image_embedding.transpose(1, 2).view(
            bs, -1, int(self.feat_shape[0]), int(self.feat_shape[1]), int(self.feat_shape[2])
        )
        # test mode
        if self.test_mode:
        ...
        sl_loss = self.supervised_forward(
            image, image_embedding, img_shape, kwargs["train_organs"], kwargs["train_labels"]
        )
        ## ssl
    ...
    def unsupervised_forward(self, image, image_embedding, pseudo_seg_cleaned, img_shape):
        ...
        for iter in range(self.decoder_iter):
            if iter % 2 == 0:
                pseudo_labels, pseudo_points_prompt = self.build_pseudo_point_prompt_label(
                    image.shape, pseudo_seg_cleaned
                )
                logits = self.forward_decoder(
                    image_embedding, img_shape, text=None, boxes=None, points=pseudo_points_prompt
                )
            else:
                pseudo_labels, pseudo_bboxes_prompt = self.build_pseudo_box_prompt_label(
                    image.shape, pseudo_seg_cleaned
                )
                logits = self.forward_decoder(
                    image_embedding, img_shape, text=None, boxes=pseudo_bboxes_prompt, points=None
                )
            # cal loss
