...
def preprocess(raw_download_dir: Path):
    ...
    for case in tqdm(list(cases)):
        case_path = os.path.join(dicom_dir, case)
        ...
        if len(subfolders) != 1:
            ...
            for sub in subfolders:
                ...
                if len(seg_scans) == 1:
                    ...
                    print(f"Case {case} has more than one segmentation!!!")
                    break
        ...
        for i, ct_scan in enumerate(ct_scans):
            ...
            if ct.SeriesInstanceUID == seg_uid:
                matches[case] = {"ct": path, "seg": seg_path}
                break
    ...
    for case, paths in matches.items():
        if (imagesTr_dir / (case + ".nrrd")).exists():
            continue
        ...
        nrrd.write(str(imagesTr_dir / f"{case}_0000.nrrd"), ct_arr, ct_header)
        ...
        seg_innrrd.to_file(labelsTr_dir / f"{case}.nrrd")
