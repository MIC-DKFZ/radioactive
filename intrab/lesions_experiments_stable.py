...
def run_experiments(
    ...
    results = {
        exp_name: {  # For now, create one dictionary. This will be split by experiment later
            split: {os.path.basename(gt): {} for _, gt in imgs_gts_split}
    ...
    if target_is_lesion:
        ...
        if not os.path.exists(instances_dict_path):
            raise RuntimeError(
                "Dataset directory must include a dataset json containing a labels dictionary detailing the organs"
            )
